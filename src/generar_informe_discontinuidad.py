...
OUT_BASE = Path("reports/predicciones_discontinuidad")

FEATURES_DISCONTINUIDAD = [
    "delta_prev_rms_mean",
    "delta_prev_zcr_mean",
    "delta_prev_spectral_centroid_mean",
    "delta_prev_spectral_bandwidth_mean",
    "delta_prev_spectral_rolloff_mean",
]

# Añadimos también cambios en MFCC
for i in range(1, 14):
    FEATURES_DISCONTINUIDAD.append(f"delta_prev_mfcc_{i}_mean")
...
def main():
    ...
    parser.add_argument("--threshold", type=float, default=0.75, help="Umbral de discontinuidad normalizada")
    ...
    disponibles = [c for c in FEATURES_DISCONTINUIDAD if c in df_audio.columns]

    if not disponibles:
        raise ValueError("No se han encontrado columnas de discontinuidad.")

    scores_norm = []

    for col in disponibles:
        scores_norm.append(normalizar_serie(df_audio[col]))

    df_audio["score_discontinuidad"] = pd.concat(scores_norm, axis=1).mean(axis=1)
    df_audio["score_suavizado"] = (
        df_audio["score_discontinuidad"]
    ...
    df_audio[[
        ...
        "fin_ventana_s",
        "centro_ventana_s",
        "score_discontinuidad",
        "score_suavizado"
    ]].to_csv(out_dir / "window_scores_discontinuidad.csv", index=False, sep=";")

    plt.figure(figsize=(12, 4))
    plt.plot(df_audio["centro_ventana_s"], df_audio["score_suavizado"], label="Score de discontinuidad")
    plt.axhline(args.threshold, linestyle="--", label=f"Umbral = {args.threshold}")
    ...
    plt.xlabel("Tiempo (s)")
    plt.ylabel("Score de discontinuidad")
    plt.title(f"Curva de discontinuidad - {audio_file}")
    plt.legend()
    plt.tight_layout()
    plt.savefig(out_dir / "score_discontinuidad.png", dpi=150)
    plt.close()
    ...
    plt.title(f"Espectrograma y discontinuidades - {audio_file}")
    plt.tight_layout()
    plt.savefig(out_dir / "spectrograma_discontinuidad.png", dpi=150)
    plt.close()

    resumen = []
    resumen.append(f"Audio analizado: {audio_file}")
    resumen.append(f"Tamper score discontinuidad: {tamper_score:.4f}")
    resumen.append(f"Umbral utilizado: {args.threshold}")
    resumen.append("")
    ...
    (out_dir / "resumen_discontinuidad.txt").write_text("\n".join(resumen), encoding="utf-8")

    print(f"Informe generado en: {out_dir}")
    print(f"Tamper score discontinuidad: {tamper_score:.4f}")
